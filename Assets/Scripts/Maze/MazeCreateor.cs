    ...
    private void GenerateMaze(int sizeX, int sizeY)
    {
        ...
        for (int x = 0; x < sizeX; x++)
        {
            for (int y = 0; y < sizeY; y++)
            {
                ...
                //¬ зависимости от того четные стороны лабиринта или не четные
                else if (x % 2 == (sizeX % 2 == 0 ? 1 : 0) && y % 2 == (sizeY % 2 == 0 ? 1 : 0))
                {
                    //— определенным шансом .1f
                    //≈сли 0 - будет много стен, если .9f - то почти пустой лабиринт
                    if (UnityEngine.Random.value > .1f)
                    {
                        //ѕытаемс€ создать стену
                        cells[x, y] = CreateCellWithType(CellType.Wall); 

                        //¬ точке соседней с текущей
